    ...
    for( auto cs : modules ){
        if( THEKERNEL->config->value(leveling_strategy_checksum, cs, enable_checksum )->as_bool() ){
            ...
            switch(cs) {
                ...
                case incremental_leveling_strategy_checksum:
                    ls= new IncrementalStrategy(this);
                    found= true;
                    break;
            }
            if(found) {
...
bool ZProbe::run_probe(float& mm, float feedrate, float max_dist, bool reverse)
{
    ...
    float pos1[3];
    float pos2[3];
    THEROBOT->get_current_machine_position(pos1);
    ...
    THEROBOT->get_current_machine_position(pos2);
    mm= pos1[2]-pos2[2];//z_start_pos - THEROBOT->actuators[2]->get_current_position();
